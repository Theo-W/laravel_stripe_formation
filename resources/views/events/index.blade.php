                    ...
                    <div class="py-8 px-4 lg:w-1/3 {{ $event->premium ? 'bg-blue-200' : '' }}">
                        <div class="h-full flex items-start">
                            <div class="w-12 flex-shrink-0 flex flex-col text-center leading-none">
                                <span
                                    class="text-gray-500 pb-2 mb-2 border-b-2 border-gray-200">{{ $event->start_at->translatedFormat('M') }}</span>
                                <span
                                    class="font-medium text-lg text-gray-800 title-font leading-none">{{ $event->start_at->translatedFormat('d') }}</span>
                            </div>
                            <div class="w-12 flex-shrink-0 flex flex-col text-center leading-none">
                                <span
                                    class="text-gray-500 pb-2 mb-2 border-b-2 border-gray-200">{{ $event->ends_at->translatedFormat('M') }}</span>
                                <span
                                    class="font-medium text-lg text-gray-800 title-font leading-none">{{ $event->ends_at->translatedFormat('d') }}</span>
                            </div>
